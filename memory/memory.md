...
- Mulda v1.0.0-rc.3 (C větev): stabilizační fix test gate — `tests/transpile.test.js` už nehardcoduje `manifest.version` na `1.0.0-rc.2`, ale bere ji z `package.json`; tím odpadají falešné pády po version bumpu, při zachování stejného C cross-build contractu.
- Mulda v1.0.0-rc.4 (C větev): proběhla plná stabilizační re-validace (`npm test`, `build:c:cross`, `release:rc`) a artifact refresh; nový bundle `release/bundles/rc-1.0.0-rc.4-20260315T201317Z` prošel integrity gate včetně `sha256sum -c release/checksums.sha256`.
- Mulda v1.0.0-rc.5 (C větev): release runtime copy (`release/linux|windows/runtime/src/mulda.js`) byla dorovnána na aktuální compile orchestrator s `getCCompileArgs`; následná full gate re-validace (`npm test`, `build:c:cross`, `release:rc`, `check:installers`, `audit:reproducibility`) PASS, nový bundle `release/bundles/rc-1.0.0-rc.5-20260315T203329Z`.
- Mulda GA (C větev): opakovaný post-freeze re-validation pass 2026-03-16 00:13Z prošel (`npm test`, `build:c:cross`, `check:installers`, `release:ga` 2x, `audit:reproducibility:ga`); strict artefakty stabilní, non-blocking drift zůstává jen v timestampovaných manifestech.
- Mulda GA (C větev): navazující hourly re-validation pass 2026-03-16 00:33Z také PASS; nové bundly `ga-1.0.0-20260316T003302Z` a `ga-1.0.0-20260316T003310Z`, commit/push stále BLOKUJE kvůli pre-existing dirty stromu repozitáře.
- Mulda GA (C větev): další hourly re-validation pass 2026-03-16 00:43Z PASS (`npm test`, `build:c:cross`, `check:installers`, `release:ga`, `audit:reproducibility:ga`); nový bundle `ga-1.0.0-20260316T004316Z`, strict artefakty stabilní, blokace commit/push beze změny (dirty monorepo strom).
- Mulda GA (C větev): hourly re-validation pass 2026-03-16 01:03Z PASS (`npm test`, `build:c:cross`, `check:installers`, `release:ga`, `audit:reproducibility:ga`); nový bundle `ga-1.0.0-20260316T010311Z`, strict artefakty stabilní, blokace commit/push stále trvá kvůli pre-existing dirty stromu monorepa.
