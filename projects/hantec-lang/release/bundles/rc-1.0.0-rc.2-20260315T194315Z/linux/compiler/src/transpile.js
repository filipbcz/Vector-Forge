...
function normalizeCExpression(expr) {
  ...
  cExpr = cExpr.replace(/\bobsahuje\s*\(([^()]*)\)/g, (_, rawArgs) => {
    const args = splitCallArgs(rawArgs);
    if (args.length !== 2) return `obsahuje(${rawArgs})`;
    return `__mulda_obsahuje(${args[0]}, ${args[1]})`;
  });

  cExpr = cExpr.replace(/\bminimum\s*\(([^()]*)\)/g, (_, rawArgs) => {
    const args = splitCallArgs(rawArgs);
    if (args.length === 0) return `minimum(${rawArgs})`;
    const converted = args.map((a) => (/^"([^"\\]|\\.)*"$/.test(a) ? `atof(${a})` : `(double)(${a})`));
    return `__mulda_minimum(${converted.length}, ${converted.join(', ')})`;
  });

  cExpr = cExpr.replace(/\bmaximum\s*\(([^()]*)\)/g, (_, rawArgs) => {
    const args = splitCallArgs(rawArgs);
    if (args.length === 0) return `maximum(${rawArgs})`;
    const converted = args.map((a) => (/^"([^"\\]|\\.)*"$/.test(a) ? `atof(${a})` : `(double)(${a})`));
    return `__mulda_maximum(${converted.length}, ${converted.join(', ')})`;
  });

  return cExpr;
}

function isLikelyBooleanCExpression(cExpr, resolveVarType) {
  const trimmed = cExpr.trim();
  if (!trimmed) return false;
  if (trimmed === 'true' || trimmed === 'false') return true;
  if (/^[A-Za-z_][A-Za-z0-9_]*$/.test(trimmed)) {
    return resolveVarType(trimmed) === 'bool';
  }
  if (trimmed.startsWith('__mulda_obsahuje(')) return true;
  ...
  function emit(nodes, indent = 0, scopeChain = [new Map()]) {
    ...
    for (const node of nodes) {
      if (node.type === 'declare') {
        ...
        const cExpr = normalizeCExpression(node.expr);
        const isStringLiteral = /^"([^"\\]|\\.)*"$/.test(cExpr);
        const isBooleanExpr = isLikelyBooleanCExpression(cExpr, resolveVarType);
        out.push(`${pad}__mulda_trace("PRINT_EXPR", ${node.line}, "");`);
        if (isStringLiteral) out.push(`${pad}printf("%s\\n", ${cExpr});`);
        else if (isBooleanExpr) out.push(`${pad}__mulda_print_bool((bool)(${cExpr}));`);
        else out.push(`${pad}__mulda_print_num((double)(${cExpr}));`);
      } else if (node.type === 'if') {
        const cExpr = normalizeCExpression(node.expr);
