...
BUNDLE_DIR="release/bundles/ga-${VERSION}-${STAMP}"
KEEP_BUNDLES="${RELEASE_KEEP_BUNDLES:-5}"

log() { printf '[release-ga] %s\n' "$*"; }
need_file() { [[ -f "$1" ]] || { echo "[release-ga] ERROR: Missing required file: $1" >&2; exit 1; }; }
...
if [[ "$KEEP_BUNDLES" =~ ^[0-9]+$ ]] && (( KEEP_BUNDLES > 0 )); then
  mapfile -t version_bundles < <(ls -1dt "release/bundles/ga-${VERSION}-"* 2>/dev/null || true)
...
log "GA bundle ready: $BUNDLE_DIR"
